    ...
    passport.authenticate('jwt', {
        ...
        User.findOne({
            ...
            .then((user) => {
                ...
                const editThesis = user.thesis
                    .map(item => item._id)
                    .indexOf(req.body.id)
                res.send({
                    thesis: user.thesis[editThesis]
                })
    ...
    passport.authenticate('jwt', {
        ...
        User.findOne({
            ...
            .then((user) => {
                ...
                const editThesisIndex = user.thesis
                    .map(item => item._id)
                    .indexOf(req.body.id)

                user.thesis[editThesisIndex].title = req.body.title
                if (typeof req.body.authors !== undefined) {
                    user.thesis[editThesisIndex].authors = req.body.authors.split(',')
                }
                if (typeof req.body.keyword !== undefined) {
                    user.thesis[editThesisIndex].keyword = req.body.keyword.split(',')
                }
                user.thesis[editThesisIndex].description = req.body.description
                user.thesis[editThesisIndex].publicationdate = req.body.publicationdate
                user.thesis[editThesisIndex].institution = req.body.institution

                var date = new Date
                user.thesis[editThesisIndex].date = date.toLocaleDateString() + " " + date.toLocaleTimeString()


                const editThesisIndexTimeline = user.timeline
                    .map(item => item.id)
                    .indexOf(req.body.id)
                user.timeline[editThesisIndexTimeline].id = req.body.id
                user.timeline[editThesisIndexTimeline].tag = 'thesis'
                user.timeline[editThesisIndexTimeline].title = req.body.title
                user.timeline[editThesisIndexTimeline].username = req.user.username
                user.timeline[editThesisIndexTimeline].date = date.toLocaleDateString() + " " + date.toLocaleTimeString()
